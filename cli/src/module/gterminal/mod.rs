...
use iced_glutin::glutin::event_loop::{ControlFlow, EventLoop};
...
use iced_glutin::glutin::platform::windows::EventLoopExtWindows;

#[cfg(target_os = "linux")]
use iced_glutin::glutin::platform::unix::EventLoopExtUnix;

pub async fn module_load(app: &App) -> anyhow::Result<()> {
    let (gl, event_loop, windowed_context, shader_version) = {
        let el: EventLoop<()> = glutin::event_loop::EventLoop::new_any_thread();
